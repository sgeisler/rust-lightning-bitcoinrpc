...
impl FromStr for NodeDescriptor {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		if !s.is_ascii() || s.len() < 67 || s.chars().nth(66) != Some('@') {
			return Err(NodeDescriptorError::FormatError)
		}

		let pk_hex = &s[..66];
		let addr_str = &s[67..];

		let pubkey = match hex_to_compressed_pubkey(pk_hex) {
			Some(pk) => pk,
			None => return Err(NodeDescriptorError::PubKeyParseError),
		};

		let addr: std::net::SocketAddr = match addr_str.parse() {
			Ok(a) => a,
			Err(std::net::AddrParseError {..}) => return Err(NodeDescriptorError::AddressParseError),
		};
			...
			match args.get(0).map(|c| *c) {
				...
				Some("c") => {
					match args.get(1).map(|arg| arg.parse()) {
						...
						None | Some(Err(NodeDescriptorError::FormatError)) => {
							println!("Invalid line, should be c pubkey@host:port");
						},
						Some(Err(NodeDescriptorError::PubKeyParseError)) => {
							println!("Bad PubKey for remote node");
						},
						Some(Err(NodeDescriptorError::AddressParseError)) => {
				...
				Some(c) => {
					if !c.chars().all(char::is_whitespace) {
						println!("Unknown command: {}. Use 'h' to get help.", c)
					}
				},
				None => {},
			}
